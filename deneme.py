...
def printTable(table,column_keys,row_keys):
    print("------------")
    row_format="{:<25}" * (len(column_keys) + 1)
...
def getRowIndex(column_key,table):
    min = 99999999
    min_index = -1
    print("COLUMN",*getColumn(column_key,table))
    for index,i in enumerate(getColumn(column_key,table)):
        if index == 0:
            continue
        if i <= 0:
            continue
        if (table[index][-1] / i) < min :
            min = table[index][-1] / i
            min_index = index
    print("MIN COLUMN ",min_index,min)
    return index
...
def twoPhaseSimplex(foo):
    ...
    while(not(isFinished(table,objsense))):
        ...
        print("pivot",column_key,row_key,table[row_key][column_key])
        table[row_key] = divisionOnce(table[row_key],table[row_key][column_key])
        print("table[%d]"%row_key,*table[row_key])
